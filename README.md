...
| `kdelinux/system/snapper` | Checks the snapper setup for the per-user home subvolumes; the files it needs in `/etc`, the home template, the config created on login, snapshots taken by the user, the qgroup and ranged limits the space aware cleanup needs, and the config, snapshots, qgroup and home going away with the user. |
